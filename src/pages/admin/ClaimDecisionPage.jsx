...
function ClaimDecisionPage() {
  ...
  const isDecidable = ['RECOMMENDED_APPROVAL', 'RECOMMENDED_REJECTION', 'UNDER_REVIEW', 'SUBMITTED'].includes(claim.claimStatus)
  const isResolved = ['APPROVED', 'REJECTED'].includes(claim.claimStatus)
  const isBusy = approveClaim.isPending || rejectClaim.isPending

  const handleApprove = () => {
    approveClaim.mutate({ id, remarks }, {
      onSuccess: () => { showToast('Claim approved.', 'success'); navigate('/admin/claims') },
      onError: (err) => handleApiError(err, showToast),
    })
  }

  const handleReject = () => {
    rejectClaim.mutate({ id, remarks }, {
      onSuccess: () => { showToast('Claim rejected.', 'success'); navigate('/admin/claims') },
      onError: (err) => handleApiError(err, showToast),
    })
  }
