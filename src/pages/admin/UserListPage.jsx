...
function UserListPage() {
  ...
  const columns = [
    { key: 'fullName', header: 'Name', render: (row) => row.fullName ?? row.email },
    { key: 'email', header: 'Email' },
    { key: 'role', header: 'Role', render: (row) => <span className="inline-flex rounded-md bg-gray-100 px-2.5 py-0.5 text-xs font-medium text-gray-700">{row.role}</span> },
    { key: 'active', header: 'Status', render: (row) => <StatusBadge status={row.active ? 'ACTIVE' : 'INACTIVE'} /> },
    {
      key: 'actions', header: 'Actions',
      render: (row) => (
        <button onClick={(e) => { e.stopPropagation(); handleToggle(row) }} disabled={toggleUser.isPending}
