...
import { useCustomer, useCustomerByUserId } from '../../hooks/useCustomers.js'
...
const POLICY_COLUMNS = [
  ...
  { key: 'planName', header: 'Plan', render: (row) => row.planName ?? row.plan?.planName ?? '—' },
  { key: 'status', header: 'Status', render: (row) => <StatusBadge status={row.status} /> },
  { key: 'startDate', header: 'Start Date', render: (row) => formatDate(row.startDate) },
]

function SharedCustomerDetailPage({ idType = 'customerId' }) {
  const { id } = useParams()

  // Use the correct hook depending on how the route passes the ID type
  const byIdQuery = useCustomer(idType === 'customerId' ? id : null)
  const byUserIdQuery = useCustomerByUserId(idType === 'userId' ? id : null)

  const { data: customerData, isLoading: customerLoading, error } = 
    idType === 'userId' ? byUserIdQuery : byIdQuery

  const customer = customerData?.data ?? customerData

  const { params, page, pageSize, setPage, setPageSize } = usePagination({ defaultSort: 'startDate' })
  
  // Resolve the CustomerId for policies lookup
  const fetchCustomerId = customer?.customerId || (idType === 'customerId' ? id : null)
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 space-y-4">
        <h2 className="text-base font-semibold text-gray-900 border-b border-gray-100 pb-3">Profile Details</h2>
...
export default SharedCustomerDetailPage